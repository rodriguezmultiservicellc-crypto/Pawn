    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3 rounded-xl border border-hairline bg-canvas p-5">
        ...
        <dl className="grid grid-cols-2 gap-y-1 text-sm">
          <dt className="text-ash">{t.portal.loans.principalOutstanding}</dt>
          <dd className="text-right font-mono text-ink">
            {formatMoney(loan.principalOutstanding)}
          </dd>
          <dt className="text-ash">{t.portal.loans.interestAccrued}</dt>
          <dd className="text-right font-mono text-ink">
        ...
        {payoffEnabled ? (
          <>
            <p className="text-xs text-ash">{t.portal.loans.payoffPanelHelp}</p>
            <button
              type="button"
              onClick={onPayClick}
              disabled={pending}
      ...
      <section className="space-y-3 rounded-xl border border-hairline bg-canvas p-5">
        ...
        <dl className="grid grid-cols-2 gap-y-2 text-sm">
          <dt className="text-ash">{t.portal.loans.principal}</dt>
          <dd className="text-right font-mono text-ink">
            {formatMoney(loan.principal)}
          </dd>
          <dt className="text-ash">{t.portal.loans.issuedOn}</dt>
          <dd className="text-right text-ink">
            {formatDateUtc(loan.issueDate)}
          </dd>
          <dt className="text-ash">{t.portal.loans.dueDate}</dt>
          <dd className="text-right text-ink">{formatDateUtc(loan.dueDate)}</dd>
          <dt className="text-ash">{t.portal.loans.collateralLine}</dt>
          <dd className="text-right text-ink">
      ...
      <section className="space-y-3 rounded-xl border border-hairline bg-canvas p-5">
        ...
        {paymentEvents.length === 0 ? (
          <p className="text-sm text-ash">{t.portal.loans.noPayments}</p>
        ) : (
