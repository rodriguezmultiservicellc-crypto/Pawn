...
export default function MagicLinkForm() {
  ...
  if (verifying) {
    return (
      <div className="space-y-3 text-center">
        ...
        <p className="text-sm text-ash">{t.auth.magicLinkPage.verifying}</p>
        {error ? (
  ...
  if (sent) {
    return (
      <div className="space-y-4 text-center">
        ...
        <p className="text-sm text-ash">{t.auth.magicLinkSent}</p>
        <button
          type="button"
          onClick={() => {
            setSent(false)
        ...
        <Link
          href="/login"
          className="block text-sm text-ash hover:text-ink"
          onClick={() => {
            router.refresh()
          }}
        >
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <p className="text-sm text-ash">{t.auth.magicLinkPage.help}</p>
