...
- **Success Green** (`#2D7A4E`) — *project extension, not extracted from Airbnb*. CSS variable `--color-success`. Active loans, paid invoices, redeemed pawn tickets, ready-for-pickup repair tickets, in-stock inventory. Saturation matched to Error Red's dusty palette so it sits comfortably next to it. Pair with `Deep Success` `#1F5535` for pressed/active states.
- **Warning Amber** (`#B7791F`) — *project extension, not extracted from Airbnb*. CSS variable `--color-warning`. Pending review, due-soon loan, repair waiting on parts, layaway falling behind, hold-period nearing expiration. Saturation matched to Error Red. Pair with `Deep Warning` `#8C5A14` for pressed/active states.
- **Translucent Black** (`rgba(0, 0, 0, 0.24)`): CSS variable `--palette-text-material-disabled`. Disabled material-style labels.

> **On the Success / Warning extension.** Airbnb's source palette intentionally has no Success Green or Warning Amber — completed states are expressed with a checkmark icon and Ink Black. That works for browse / detail surfaces but breaks in dense financial tables (a 6-column loan list with 80 rows can't communicate state through icon-only differentiation). The extension above is the smallest possible addition: two new semantic tokens plus their pressed-state variants, saturation matched to the existing Error Red so they don't visually fight the rest of the system. They are strictly semantic — never decorative, never used for emphasis, never used outside status communication.
